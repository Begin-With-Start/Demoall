...
/**
 * 作者：minifly on 2016/11/29 16:50
 * 一个别人的闹钟
 */
public class CanvasViewClock extends View{
    private Context context;

    public CanvasViewClock(Context context) {
        super(context);
        this.context = context;
    }

    public CanvasViewClock(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
    }

    public CanvasViewClock(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        canvas.drawBitmap(bitmap, 250,360, p);
        ...
        Log.e("message","" + DensityUtils.px2dip(context,this.getWidth()) + "   " +  DensityUtils.px2dip(context,this.getHeight()));//获取到的就是我们在布局里面设置的dp高度与宽度。
        ...
        for(int i=0 ; i <count ; i++){
            if(i%5 == 0){
                ...
                canvas.drawText(String.valueOf(i/5+1), -5f, y+lineLengthLong + 5, tmpPaint);

            }else{
                canvas.drawLine(0f, y, 0f, y +lineLengthShort, tmpPaint);
            }
