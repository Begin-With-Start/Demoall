...
public class MyCircleProgressView extends View {
	...
	private void init(Context c) {
		...
		mPaintJinji.setColor(mContext.getResources().getColor(R.color.task_blue));
		mPaintJinji.setStrokeWidth(dip2px(mContext, 8));
		mPaintJinji.setAntiAlias(true);//去锯齿
		mPaintJinji.setStrokeCap(Paint.Cap.BUTT);//圆形的笔触
		
		// 普通笔头设置
		mPaintNormal = new Paint();
		mPaintNormal.setStyle(Paint.Style.STROKE);
		mPaintNormal.setColor(mContext.getResources().getColor(R.color.gray));
		mPaintNormal.setStrokeWidth(dip2px(mContext, 8));
		mPaintNormal.setAntiAlias(true);
		mPaintNormal.setStrokeCap(Paint.Cap.BUTT);
...
//		mPaintTimeout.setAntiAlias(true);
	...
	protected void onDraw(Canvas canvas) {
        ...
        LinearGradient shader = new LinearGradient(x - radius, y - radius, x + radius, y + radius, new int[]{R.color.task_blue,Color.WHITE}, null, Shader.TileMode.REPEAT);
		mPaintJinji.setShader(shader);
